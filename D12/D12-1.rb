...
def energy(points)
  points.each_with_object([]) do |c, acc|
    acc << c[:x].abs + c[:y].abs + c[:z].abs
  end
end

def total_energy(potential, kinetic)
  nrj = 0
  (0..potential.length - 1).each do |index|
    nrj += potential[index] * kinetic[index]
  end
  nrj
end
...
result_total = total_energy(energy(moons), energy(vel))
puts result_total
